...
const Suggestion = () => {
  ...
  const { inputSearch, setInputSearch } = context;
  ...
  const fetchSuggestions = async () => {
    ...
    setError(null);
    try {
      console.log("Check value input", inputSearch);

      const input = inputSearch;
      console.log("Check value input after", input);

      const response = await axiosInstance.post("/search/find_recipes", null, {
        params: {
          input_ingredients: Array.isArray(input)
            ? input.map((ing) => ing.trim()).join(",")
            : input,
        },
      });
  ...
  const handleSearch = () => {
    if (loading) return;
    if (!inputSearch) {
      message.error("Vui lòng nhập ít nhất một nguyên liệu để tìm kiếm!");
      return;
    }
