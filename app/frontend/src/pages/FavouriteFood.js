...
const FavouriteFood = () => {
    ...
    const fetchFoods = async () => {
      try {
        const response = await axiosInstance.get(
          `/favourite_food/favourite/${user_id}`
        );
  ...
  const deleteItem = async (food_id) => {
    try {
      ...
      setFoods((prevFoods) =>
        prevFoods.filter((food) => food.food_id !== food_id)
      );
    } catch (err) {
  ...
  return (
    <div className="container">
      <h1 className="text-2xl font-bold mb-4 flex items-center gap-2">
        Món Ăn Yêu Thích <FaHeart className="titleIcon" />
      </h1>
      {foods.length === 0 ? (
              ...
              <div className="foodInfo">
                ...
                <div className="buttonGroup">
                  ...
                  <button
                    ...
                    onClick={() => deleteItem(food.food_id)}
                  >
                    <FaTrash /> Xóa
                  </button>
                </div>
      ...
      >
        {selectedFood && (
          <div className="foodDetails">
            <img
              src={selectedFood.image || imgdefault}
            ...
            <div className="modalContent">
              ...
              <p
                dangerouslySetInnerHTML={{
                  __html: selectedFood.description || "Không có mô tả.",
                }}
              />
              <h3>Nguyên liệu:</h3>
              <ul>
              ...
              <p
                dangerouslySetInnerHTML={{
                  __html: selectedFood.instructions || "Không có hướng dẫn.",
                }}
              />
            </div>
          </div>
